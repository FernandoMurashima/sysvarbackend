...
class EstoqueAcessoLojaApiTests(TestCase):
    ...
    def test_indicadores_inventario_pendentes_zero_correto_sobra_e_falta(self):
        inv = InventarioEstoque.objects.create(Idloja=self.loja_1, descricao="Indicadores")
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000931", referencia="PEND", saldo_sistema=Decimal("3.000"), contado=False)
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000932", referencia="ZERO", saldo_sistema=Decimal("0.000"), saldo_contado=Decimal("0.000"), contado=True)
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000933", referencia="OK", saldo_sistema=Decimal("2.000"), saldo_contado=Decimal("2.000"), contado=True)
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000934", referencia="SOBRA", saldo_sistema=Decimal("1.000"), saldo_contado=Decimal("4.000"), contado=True)
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000935", referencia="FALTA", saldo_sistema=Decimal("5.000"), saldo_contado=Decimal("1.000"), contado=True)

        self.client.force_authenticate(self.restrito)
        resp = self.client.get(f"/api/produto/inventario-estoque/{inv.pk}/")

        self.assertEqual(resp.status_code, 200, resp.content)
        self.assertEqual(resp.data["total_itens"], 5)
        self.assertEqual(resp.data["total_contados"], 4)
        self.assertEqual(resp.data["total_pendentes"], 1)
        self.assertEqual(resp.data["total_sem_divergencia"], 2)
        self.assertEqual(resp.data["total_sobra"], 1)
    ...
    def test_finalizacao_bloqueia_pendencias_e_ajusta_estoque_fisico(self):
        ...
        contado = InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra=sku.ean13, referencia=produto.referencia, saldo_sistema=Decimal("8.000"), saldo_contado=Decimal("6.000"), contado=True)
        InventarioEstoqueItem.objects.create(inventario=inv, CodigodeBarra="7890000000936", referencia="PEND", saldo_sistema=Decimal("1.000"), contado=False)
        movs_antes = EstoqueMovimentacao.objects.count()

        self.client.force_authenticate(self.restrito)
        bloqueado = self.client.post(f"/api/produto/inventario-estoque/{inv.pk}/finalizar/", {}, format="json")
        self.assertEqual(bloqueado.status_code, 400, bloqueado.content)

        pendente = inv.itens.get(CodigodeBarra="7890000000936")
        pendente.saldo_contado = Decimal("0.000")
        pendente.contado = True
        pendente.save()
        permitido = self.client.post(f"/api/produto/inventario-estoque/{inv.pk}/finalizar/", {}, format="json")

        self.assertEqual(permitido.status_code, 200, permitido.content)
        inv.refresh_from_db()
    ...
    def test_consulta_referencia_filtra_por_colecao_e_sem_colecao_retorna_todas_permitidas(self):
        produto_a, sku_a, _, _, colecao_a = self.criar_produto_com_sku(referencia_esperada="260101771", colecao_codigo="26", estacao="01")
        produto_b, sku_b, _, _, _ = self.criar_produto_com_sku(referencia_esperada="270101772", colecao_codigo="27", estacao="01")
        Estoque.objects.create(CodigodeBarra=sku_a.ean13, referencia=produto_a.referencia, Idloja=self.loja_1, Estoque="4.000", reserva="0.000")
        ...
        resp_sem_colecao = self.client.get("/api/produto/estoque/consulta-referencia/", {"referencia": produto_b.referencia})
        self.assertEqual(resp_sem_colecao.status_code, 200, resp_sem_colecao.content)
        self.assertEqual({row["referencia"] for row in self.results(resp_sem_colecao)}, {produto_b.referencia})

    def test_consulta_referencia_com_colecao_preserva_isolamento_de_empresa_e_loja(self):
        produto, sku, _, _, colecao = self.criar_produto_com_sku(referencia_esperada="260101773", colecao_codigo="26", estacao="01")
        ...
        Estoque.objects.create(CodigodeBarra=sku.ean13, referencia=produto.referencia, Idloja=self.loja_2, Estoque="6.000", reserva="0.000")
        unidade_b = Unidade.objects.create(empresa=self.outra_empresa, Descricao="Unidade B", Codigo="UB")
        colecao_b = Colecao.objects.create(empresa=self.outra_empresa, Descricao="Colecao B", Codigo="26", Estacao="01")
        produto_b = Produto.objects.create(empresa=self.outra_empresa, tipo_produto="1", referencia="260101999", descricao="Outro", descricao_reduzida="Outro", unidade=unidade_b, colecao=colecao_b)
        Estoque.objects.create(CodigodeBarra="7890000000773", referencia=produto_b.referencia, Idloja=self.loja_outra_empresa, Estoque="99.000", reserva="0.000")

        self.client.force_authenticate(self.restrito)
        resp = self.client.get("/api/produto/estoque/consulta-referencia/", {"colecao": colecao.Idcolecao})

        self.assertEqual(resp.status_code, 200, resp.content)
        rows = self.results(resp)
        self.assertEqual({row["loja"] for row in rows}, {self.loja_1.id})
        self.assertNotIn(self.loja_outra_empresa.id, {row["loja"] for row in rows})

    def test_consulta_referencia_com_loja_nao_retorna_referencia_apenas_de_outra_loja(self):
        produto, sku, _, _, _ = self.criar_produto_com_sku(referencia_esperada="260101777")
        Estoque.objects.create(CodigodeBarra=sku.ean13, referencia=produto.referencia, Idloja=self.loja_2, Estoque="4.000", reserva="0.000")

        self.client.force_authenticate(self.master)
        resp = self.client.get("/api/produto/estoque/consulta-referencia/", {"referencia": produto.referencia, "loja": self.loja_1.id})

        self.assertEqual(resp.status_code, 200, resp.content)
        self.assertEqual(self.results(resp), [])

    def test_usuario_restrito_nao_ve_referencia_de_loja_nao_permitida_na_consulta_especifica(self):
        produto, sku, _, _, _ = self.criar_produto_com_sku(referencia_esperada="260101778")
        Estoque.objects.create(CodigodeBarra=sku.ean13, referencia=produto.referencia, Idloja=self.loja_2, Estoque="4.000", reserva="0.000")
    ...
    def test_uso_consumo_respeita_loja_permitida_e_mantem_inativo_com_saldo(self):
        ...
        rows = self.results(resp)
        self.assertEqual({row["loja"] for row in rows}, {self.loja_1.id})
        self.assertEqual({row["produto"] for row in rows}, {produto_loja_1.Idproduto})
        self.assertFalse(rows[0]["produto_ativo"])
