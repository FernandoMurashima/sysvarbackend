...
FORBIDDEN_OPERATIONAL_MODELS=[('accounts','SessaoUsuario'),('accounts','SessionToken'),('compras','Requisicao'),('compras','RequisicaoItem'),('compras','RequisicaoHistorico'),('compras','OrdemServico'),('compras','Cotacao'),('compras','PedidoCompra'),('distribuicao','Distribuicao'),('distribuicao','PedidoVendaDistribuicao'),('distribuicao','MercadoriaTransito'),('financeiro','CashbackMovimento'),('financeiro','ValeTroca'),('financeiro','ValeTrocaMovimento'),('financeiro','MovimentacaoFinanceira'),('financeiro','Pagar'),('financeiro','PagarItem'),('financeiro','Receber'),('financeiro','ReceberItem'),('fiscal','AgenteLocalSysvar'),('fiscal','AtivacaoAgenteLocalSysvar'),('fiscal','ConfiguracaoXmlFornecedor'),('fiscal','XmlFornecedorRecebido'),('fiscal','RecebimentoMercadoriaEstoque'),('fiscal','RecebimentoMercadoriaPedido'),('fiscal','RecebimentoMercadoriaConferenciaItem'),('fiscal','RecebimentoMercadoriaTermo'),('fiscal','RecebimentoMercadoriaEfetivacaoEstoque'),('fiscal','NotaFiscalEntrada'),('fiscal','NotaFiscalEntradaItem'),('fiscal','NotaFiscalEntradaItemXml'),('fiscal','NotaFiscalEntradaEvento'),('fiscal','NotaFiscalEntradaDivergenciaXml'),('fiscal','NotaFiscalSaida'),('fiscal','VendaPdv'),('fiscal','VendaDevolucao'),('fiscal','NFCe'),('produto','EstoqueMovimentacao'),('produto','ProdutoUsoConsumoMovimentacao'),('produto','InventarioEstoque'),('produto','OrdemProducao')]
TPAG_POR_TIPO_FORMA={FormaPagamento.TIPO_DINHEIRO:'01',FormaPagamento.TIPO_CREDITO:'03',FormaPagamento.TIPO_DEBITO:'04',FormaPagamento.TIPO_PIX:'17',FormaPagamento.TIPO_BOLETO:'15',FormaPagamento.TIPO_TRANSFERENCIA:'18',FormaPagamento.TIPO_OUTRO:'99'}
@dataclass
class DevBaseReport:
...
class SysvarDevBaseService:
    ...
    def financeiro(self):
        ...
        for f in self.seed('formas_pagamento.json'):
            d=dict(f); d['empresa']=emp; d['prazo_pagamento']=self.i['prazo'].get(d.pop('prazo_codigo',None)); obj,_=FormaPagamento.objects.update_or_create(empresa=emp,codigo=d['codigo'],defaults=self.clean(FormaPagamento,d)); self.i['forma'][obj.codigo]=obj; self.mapa_fiscal_forma_pagamento(obj)
    ...
    def mapa_fiscal_forma_pagamento(self,forma):
        codigo_tpag=TPAG_POR_TIPO_FORMA.get(forma.tipo)
        if not codigo_tpag: return
        FormaPagamentoFiscalMap.objects.get_or_create(empresa=forma.empresa,codigo_tpag=codigo_tpag,defaults={'forma_pagamento':forma,'descricao_fiscal':TPAG_DESCRICOES.get(codigo_tpag,forma.descricao),'ativo':True})
    ...
    def forbidden(self):
        ...
        return labels
    def delete_all(self):
        self.delm([SessionToken, AtivacaoAgenteLocalSysvar, RecebimentoMercadoriaEfetivacaoEstoque, RecebimentoMercadoriaTermo, RecebimentoMercadoriaConferenciaItem, RecebimentoMercadoriaPedido, NotaFiscalEntradaDivergenciaXml, NotaFiscalEntradaEvento, NotaFiscalEntradaItem, NotaFiscalEntradaItemXml, NotaFiscalSaidaItem, NFeDevolucao, NFCe, VendaDevolucaoItem, ValeTrocaMovimento, CashbackMovimento, VendaPdvPagamento, VendaPdvItem, AntecipacaoRecebivelItem, LancamentoContabil, PagarRateio, ReceberRateio, PagarItem, ReceberItem, PedidoCompraEntrega, PedidoCompraParcela, PedidoCompraItem, CotacaoPropostaItem, CotacaoProposta, CotacaoRequisicao, CotacaoItem, CotacaoFornecedor, OrdemServicoMaterial, RequisicaoHistorico, RequisicaoItem, MercadoriaTransito, PedidoVendaDistribuicaoItem, DistribuicaoDestino, DistribuicaoItem, OrdemProducaoGrade, OrdemProducaoItem, InventarioEstoqueItem])
        self.delm([ValeTroca, VendaDevolucao, VendaPdv, NotaFiscalSaida, NotaFiscalEntrada, RecebimentoMercadoriaEstoque, XmlFornecedorRecebido, AgenteLocalSysvar, ConfiguracaoXmlFornecedor, AntecipacaoRecebivel, MovimentacaoFinanceira, Pagar, Receber, PedidoVendaDistribuicao, Distribuicao, OrdemServico, Cotacao, PedidoCompra, Requisicao, OrdemProducao, InventarioEstoque])
